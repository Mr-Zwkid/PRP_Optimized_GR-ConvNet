...
class GraspDatasetBase(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        if self.random_rotate:
            rotations = [0, np.pi / 2, 2 * np.pi / 2, 3 * np.pi / 2]  # 在其中随机挑选角度进行旋转
        ...
        pos_img, ang_img, width_img = bbs.draw((self.output_size, self.output_size))  # 将ground true bounding box转换成所需图片
        width_img = np.clip(width_img, 0.0, self.output_size / 2) / (self.output_size / 2)  # width先限制在(0, 150)，再归一化到(0, 1)

        if self.include_depth and self.include_rgb:
            x = self.numpy_to_torch(
                np.concatenate(
                    (np.expand_dims(depth_img, 0),  # (224, 224) -> (1, 224, 224)
                     rgb_img),                      # (3, 224, 224)
                    0
                )                                   # (4, 224, 224)
            )
