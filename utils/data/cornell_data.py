...
class CornellDataset(GraspDatasetBase):
    ...
    def __init__(self, file_path, ds_rotate=0, **kwargs):
        ...
        self.grasp_files = glob.glob(os.path.join(file_path, '*', 'pcd*cpos.txt'))  # 取出点云数据和正向抓取矩形标签
    ...
    def get_gtbb(self, idx, rot=0, zoom=1.0):
        gtbbs = grasp.GraspRectangles.load_from_cornell_file(self.grasp_files[idx])  # ground true bounding boxes
    ...
    def get_rgb(self, idx, rot=0, zoom=1.0, normalise=True):
        ...
        rgb_img.zoom(zoom)
        rgb_img.resize((self.output_size, self.output_size))  # resize preserve_range 所以(224, 224, 3)
        if normalise:
            rgb_img.normalise()
            rgb_img.img = rgb_img.img.transpose((2, 0, 1))    # (3, 224, 224)
        return rgb_img.img
