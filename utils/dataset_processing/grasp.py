...
class GraspRectangles:
    ...
    def load_from_cornell_file(cls, fname):
        ...
        with open(fname) as f:
            while True:
                ...
                try:
                    gr = np.array([
                        _gr_text_to_no(p0),     # [y1,x1]
                        _gr_text_to_no(p1),     # [y2,x2]
                        _gr_text_to_no(p2),     # [y3,x3]
                        _gr_text_to_no(p3)      # [y4,x4]
                    ])
    ...
    def draw(self, shape, position=True, angle=True, width=True):
        ...
        for gr in self.grs:
            rr, cc = gr.compact_polygon_coords(shape)  # 取出抓取矩形中像素的横纵坐标rr, cc。并以此为引索，在这些像素点上赋值
            if position:
                pos_out[rr, cc] = 1.0  # 将矩形框内像素标记为1（其余为0）
            if angle:
                ang_out[rr, cc] = gr.angle  # 将矩形框内像素标记为角度(其余为0)
            if width:
                width_out[rr, cc] = gr.length
                # 修改 (要获取宽度，但实际获取的为length)->
                # width_out[rr, cc] = gr.width  # 将矩形框内像素标记为宽度值(其余为0)
                # <- 修改
...
class GraspRectangle:
    ...
    def angle(self):
        ...
        return (np.arctan2(-dy, dx) + np.pi / 2) % np.pi - np.pi / 2  # 返回抓取角度且不需要条件判断语句
    ...
    def center(self):
        ...
        """
        return self.points.mean(axis=0).astype(np.int)  # 对各列求均值，返回 1*n矩阵，即矩阵中点
    ...
    def compact_polygon_coords(self, shape=None):
        ...
        """
        return Grasp(self.center, self.angle, self.length / 3, self.width).as_gr.polygon_coords(shape)  # 取抓取矩形的三分之一
...
class Grasp:
    ...
    @property
    def as_gr(self):  # 根据(x, y, theta, l, w)五维抓取坐标返回抓取矩形
        """
        ...
        return GraspRectangle(np.array(
            [
                [y1 - self.width / 2 * xo, x1 - self.width / 2 * yo],  # 右下角
                [y2 - self.width / 2 * xo, x2 - self.width / 2 * yo],  # 左下角
                [y2 + self.width / 2 * xo, x2 + self.width / 2 * yo],  # 左上角
                [y1 + self.width / 2 * xo, x1 + self.width / 2 * yo],  # 右上角
            ]
        ).astype(np.float))

    def max_iou(self, grs):  # 返回预测抓取矩形和所有ground true抓取矩形对比中最大的IOU
        """
    ...
    def plot(self, ax, color=None):  # 画出抓取矩形
        """
        Plot Grasp
        :param ax: Existing matplotlib axis
